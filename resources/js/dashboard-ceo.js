...
(function () {
    ...
    function waitForCanvasAndInit(retries = 12, delay = 150) {
        const topCanvas = document.getElementById("topPerformersChart");
        const phaseCanvas = document.getElementById("phaseProgressChart");
        const isVisible = (el) => {
            if (!el) return false;
        ...
        if (isVisible(topCanvas) || isVisible(phaseCanvas)) {
            initCharts();
    ...
    document.addEventListener("livewire:navigated", waitForCanvasAndInit);
    ...
    document.addEventListener("update-top-performers-chart", (event) => {
        const data = Array.isArray(event.detail)
            ? event.detail[0]
            : event.detail;
        window.__dashboardTopPerformersData = data.data;
        if (window.topPerformersChartInstance) {
            const newData = data.data;
            window.topPerformersChartInstance.data.labels = newData.map(
                (p) => p.user_name,
            );
            window.topPerformersChartInstance.data.datasets[0].data =
                newData.map((p) => p.final_score);
