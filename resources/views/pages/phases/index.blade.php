...
new #[Title('Quản lý giai đoạn')] class extends Component {
    ...
    public function ganttData(): array
    {
        ...
        $starts = $phases
            ->map(fn ($p) => $p->start_date ? Carbon::parse($p->start_date) : null)
            ->filter()->values();
        $ends = $phases
            ->map(fn ($p) => $p->end_date ? Carbon::parse($p->end_date) : null)
            ->filter()->values();
        ...
        for ($i = 0; $i < $totalDays; $i++) {
            ...
            if ($isMonthStart || ($i % $step === 0)) {
                ...
                $left = $isMonthStart
                    ? ($monthLeftMap[$dayCursor->format('Y-m')] ?? round(($i / $totalDays) * 100, 4))
...
<div>
    ...
    <div class="mb-8 grid grid-cols-1 gap-4 md:grid-cols-3">
        ...
        <div class="flex flex-col gap-1 rounded-xl border border-slate-200 bg-white p-5 dark:border-slate-800 dark:bg-slate-900">
            ...
            <div class="flex items-baseline gap-2">
                ...
                @if ($this->totalWeight == 100)
                    <span class="rounded-full bg-green-100 px-2 py-0.5 text-xs font-bold text-green-700 dark:bg-green-900/30 dark:text-green-400">Đã cân đối</span>
                @else
                    <span class="rounded-full bg-amber-100 px-2 py-0.5 text-xs font-bold text-amber-700 dark:bg-amber-900/30 dark:text-amber-400">Chưa cân đối</span>
                @endif
            </div>
        </div>
        ...
        <div class="flex flex-col gap-1 rounded-xl border border-slate-200 bg-white p-5 dark:border-slate-800 dark:bg-slate-900">
            ...
            <span class="text-2xl font-bold text-slate-900 dark:text-white">{{ $this->phases->count() }} Giai đoạn</span>
        </div>
        ...
        <div class="flex flex-col gap-1 rounded-xl border border-slate-200 bg-white p-5 dark:border-slate-800 dark:bg-slate-900">
            ...
            <div class="flex items-center gap-3">
                ...
                <div class="h-2 flex-1 overflow-hidden rounded-full bg-slate-100 dark:bg-slate-800">
                    <div class="bg-primary h-full transition-all duration-500" style="width: {{ $project->progress }}%"></div>
                </div>
            </div>
        </div>
    </div>
        ...
        <x-ui.modal wire:model="showDeleteModal" maxWidth="md">
            ...
            <div class="space-y-3">
                <p class="text-center text-sm text-slate-600 dark:text-slate-300">
                    @if ($action === 'delete') Bạn có chắc chắn muốn xóa giai đoạn này không?
                    @elseif ($action === 'start') Bạn có chắc chắn muốn bắt đầu giai đoạn này không?
                    @else Bạn có chắc chắn muốn đánh dấu giai đoạn này là hoàn thành không?
                    @endif
                </p>
            ...
            <x-slot name="footer">
                ...
                <x-ui.button
                    ...
                    :icon="$confirmIcon"
                    wire:click="performPendingPhaseAction"
                    loading="performPendingPhaseAction"
