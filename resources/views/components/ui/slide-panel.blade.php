...
<div x-data="{ isOpen: @entangle($wireModel).live }" x-effect="document.body.classList.toggle('overflow-hidden', isOpen)"
    {{ $attributes->except(['maxWidth', 'closeable', 'position', 'closeOutside'])->whereDoesntStartWith('wire:model') }}>
    <template x-teleport="body">
        <div x-show="isOpen" class="fixed inset-0 z-50 flex">
            {{-- Backdrop --}}
            ...
            {{-- Panel --}}
            <div x-show="isOpen" x-transition:enter="transform transition ease-out duration-300"
                x-transition:enter-start="{{ $translateStart }}" x-transition:enter-end="translate-x-0"
