    ...
    >
        ...
        >
            ...
            >
                ...
                >
                    <div
                        class="bg-primary/30 flex h-5 w-5 items-center justify-center rounded-full text-2xs font-bold"
                ...
                >
                    <span
                        class="material-symbols-outlined text-xs leading-none"
                    >close</span>
                </button>
            </div>
        </template>
        ...
        >
            ...
            >
                <div
                    class="border-b border-slate-100 p-2 dark:border-slate-800"
                >
                    <div
                        class="relative"
                    >
                        <span
                            class="material-symbols-outlined pointer-events-none absolute left-2.5 top-1/2 -translate-y-1/2 text-base text-slate-400"
                        >{{ $searchIcon }}</span>
                        <input
                            x-ref="searchInput"
                            x-model="search"
                ...
                <div
                    class="custom-scrollbar max-h-52 overflow-y-auto"
                >
                    <template
                        x-if="filtered.length === 0"
                    >
                        <div
                            class="px-4 py-6 text-center text-sm text-slate-400"
                        >
                            <span
                                class="material-symbols-outlined mb-1 block text-2xl"
                            >{{ $emptyIcon }}</span>
                            {{ $emptyText }}
                        </div>
                    </template>
                    ...
                    >
                        ...
                        >
                            ...
                            >
                                ...
                                >
                                    <div
                                        class="bg-primary/20 text-primary flex h-8 w-8 shrink-0 items-center justify-center rounded-full text-sm font-bold"
                                        x-text="(user.name?.charAt(0) ?? '?').toUpperCase()"
                                    ></div>
                                </template>
