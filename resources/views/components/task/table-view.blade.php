...
>
            ...
            @forelse ($tasks as $task)
                ...
                >
                    ...
                    <x-ui.table.cell>
                        @if ($task->deadline)
                            ...
                            >
                                ...
                                @if ($isOverdue)
                                    <span
                                        class="material-symbols-outlined align-middle text-xs"
                                    >warning</span>
                                @endif
                            </div>
                        @else
                    ...
                    <x-ui.table.cell>
                        ...
                        >
                            <span
                                class="{{ $status->dotClass() }} h-2 w-2 shrink-0 rounded-full"
                            ></span>
                            <span
                                class="{{ $status->badgeClass() }} rounded-full px-2 py-0.5 text-xs font-semibold"
                            >
                                {{ $status->label() }}
                            </span>
                        </div>
                    ...
                    >
                        ...
                        >
                            ...
                            @can('view', $task)
                                <x-ui.icon-button
                                    :icon="auth()->user()->can('update', $task) ? 'edit' : 'visibility'"
                                    size="sm"
                                    color="slate"
                                    :tooltip="auth()->user()->can('update', $task) ? 'Chỉnh sửa' : 'Xem chi tiết'"
                                    wire:click="openEditTask({{ $task->id }})"
                                />
                            @endcan
                            @can('delete', $task)
    ...
    >
        @can('create', App\Models\Task::class)
            <button
                @click="$dispatch('task-create-requested')"
        ...
        >
            ...
            @if ($taskStats['in_progress'] > 0)
                <span
                    class="text-primary"
                >{{ $taskStats['in_progress'] }} đang làm</span>
            @endif
        </div>
    </div>
</div>
